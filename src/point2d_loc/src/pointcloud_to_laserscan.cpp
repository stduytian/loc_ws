...
public:
  PointCloudTFNode() : Node("pointcloud_to_laserscan"), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_)
  {
    ...
    // 支持直接传入完整的 yaml 和 image 路径，如果没有则使用 pgm_path_ + filename
    std::string yaml_file = !yaml_file_param_.empty() ? yaml_file_param_ : (pgm_path_ + "office2.yaml");
    std::string image_file = !image_file_param_.empty() ? image_file_param_ : (pgm_path_ + "office2.pgm");
    ...
    rclcpp::QoS map_qos(1); // KeepLast(1)，只需最新地图
    ...
    align_flag_pub_ = this->create_publisher<std_msgs::msg::Bool>("/align_flag", rclcpp::QoS(rclcpp::KeepLast(10)).reliable());
    ...
    // 创建 TF 持续发布 timer（10Hz，保持 transform 在 TF 树中）
    tf_pub_timer_ = this->create_wall_timer(std::chrono::milliseconds(100),
                                            std::bind(&PointCloudTFNode::publishTFTransform, this));
  }

private:
  // 定期发布 TF transform（保持 transform 在 TF 树中有效）
  void publishTFTransform()
  {
    std::lock_guard<std::mutex> lock(tf_mutex_);
    if (has_valid_tf_)
    {
      latest_tf_.header.stamp = this->get_clock()->now();
  ...
  void poseCallback(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
  {
    ...
    RCLCPP_INFO(this->get_logger(), "ICP hasConverged: %s, fitness: %.6f",
    ...
    if (icp.hasConverged())
    {
      ...
      // 广播map camera_init的tf（存储以便定期发布）
      {
        std::lock_guard<std::mutex> lock(tf_mutex_);
        latest_tf_.header.stamp = this->get_clock()->now();
        latest_tf_.header.frame_id = "map";
        latest_tf_.child_frame_id = "camera_init";
        // latest_tf_.transform.translation.x = aligned_x;
        // latest_tf_.transform.translation.y = aligned_y;
        // latest_tf_.transform.translation.z = 0.0;
        // latest_tf_.transform.rotation = q_msg;
        latest_tf_.transform.translation.x = 0.0;
        latest_tf_.transform.translation.y = 0.0;
        latest_tf_.transform.translation.z = 0.26763;
        latest_tf_.transform.rotation.x = 0.0;
        latest_tf_.transform.rotation.y = 0.0;
        latest_tf_.transform.rotation.z = 0.0;
        latest_tf_.transform.rotation.w = 1.0;
        has_valid_tf_ = true;

        // RCLCPP_INFO(this->get_logger(), "Stored TF: map -> camera_init (%.2f, %.2f, %.4f)",
  ...
  nav_msgs::msg::OccupancyGrid loadMap(const std::string &pgm_file, double resolution, const std::string &override_image_file = "")
  {
    YAML::Node config = YAML::LoadFile(pgm_file);
    meta.image_file = config["image"].as<std::string>();
    // 如果外部传入了完整 image 路径，优先使用；否则用 pgm_path_ + 配置中的文件名
    std::string image_file = !override_image_file.empty() ? override_image_file : (pgm_path_ + meta.image_file);
